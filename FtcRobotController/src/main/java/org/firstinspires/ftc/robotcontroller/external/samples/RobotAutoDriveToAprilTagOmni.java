...
public class RobotAutoDriveToAprilTagOmni extends LinearOpMode {
	...
	// larger for a more
	// aggressive response.
	final double SPEED_GAIN = 0.02; // Forward Speed Control "Gain". e.g. Ramp up to 50% power at a 25 inch error.
									// (0.50
	// / 25.0)
	final double STRAFE_GAIN = 0.015; // Strafe Speed Control "Gain". e.g. Ramp up to 37% power at a 25 degree Yaw
										// error.
	// (0.375 / 25.0)
	final double TURN_GAIN = 0.01; // Turn Control "Gain". e.g. Ramp up to 25% power at a 25 degree error. (0.25 /
									// 25.0)
	...
	public void runOpMode() {
		...
		while (opModeIsActive()) {
			...
			// If Left Bumper is being pressed, AND we have found the desired target, Drive
			// to target
			// Automatically .
	...
	private void initAprilTag() {
		...
		if (USE_WEBCAM) {
			...
			visionPortal = new VisionPortal.Builder().setCamera(BuiltinCameraDirection.BACK).addProcessor(aprilTag)
					.build();
		}
	}

	/*
	 * Manually set the camera gain and exposure. This can only be called AFTER
	 * calling initAprilTag(), and only works for Webcams;
	 */
