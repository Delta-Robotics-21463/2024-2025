...
public class ConceptMotorBulkRead extends LinearOpMode {
	...
	public void runOpMode() {
		...
		// Important Step 1: Make sure you use DcMotorEx when you instantiate your
		// motors.
		m1 = hardwareMap.get(DcMotorEx.class, "m1"); // Configure the robot to use these 4 motor names,
		m2 = hardwareMap.get(DcMotorEx.class, "m2"); // or change these strings to match your existing Robot
														// Configuration.
		m3 = hardwareMap.get(DcMotorEx.class, "m3");
		m4 = hardwareMap.get(DcMotorEx.class, "m4");

		// Important Step 2: Get access to a list of Expansion Hub Modules to enable
		// changing caching
		// methods.
		...
		// In this mode, a single read is done for each encoder position, and a bulk
		// read is done for
		// each velocity read.
		...
		// In this mode, only one bulk read is done per cycle, UNLESS you read a
		// specific
		// encoder/velocity item AGAIN in that cycle.
		// --------------------------------------------------------------------------------------

		// Important Step 3: Option A. Set all Expansion hubs to use the AUTO Bulk
		// Caching mode
		...
		// This is the MOST efficient method, but it does require that the cache is
		// cleared manually
		// each control cycle.
		// --------------------------------------------------------------------------------------

		// Important Step 3: Option B. Set all Expansion hubs to use the MANUAL Bulk
		// Caching mode
		for (LynxModule module : allHubs) {
