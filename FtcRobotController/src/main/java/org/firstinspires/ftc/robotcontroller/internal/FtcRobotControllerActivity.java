...
public class FtcRobotControllerActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		RobotLog.onApplicationStart(); // robustify against onCreate() following onDestroy() but using the
		...
		// Check if this RC app is from a later FTC season than what was installed
		// previously
		int ftcSeasonYearOfPreviouslyInstalledRc = preferencesHelper
				.readInt(getString(R.string.pref_ftc_season_year_of_current_rc), 0);
		int ftcSeasonYearOfCurrentlyInstalledRc = AppUtil.getInstance()
				.getFtcSeasonYear(AppUtil.getInstance().getLocalSdkBuildMonth()).getValue();
		if (ftcSeasonYearOfCurrentlyInstalledRc > ftcSeasonYearOfPreviouslyInstalledRc) {
			preferencesHelper.writeIntPrefIfDifferent(getString(R.string.pref_ftc_season_year_of_current_rc),
					ftcSeasonYearOfCurrentlyInstalledRc);
			// Since it's a new FTC season, we should reset certain settings back to their
			// default values.
			...
			preferencesHelper.writeBooleanPrefIfDifferent(getString(R.string.pref_warn_about_mismatched_app_versions),
					true);
		...
		buttonMenu.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				AnnotatedHooksClassFilter.getInstance().callOnCreateMenuMethods(FtcRobotControllerActivity.this,
						popupMenu.getMenu());
				popupMenu.show();
			}
		});
	...
	protected void onResume() {
		...
		// In case the user just got back from fixing their clock, refresh
		// ClockWarningSource
	...
	protected void readNetworkType() {
		...
		if (Device.isRevControlHub() == true) {
			...
			networkType = NetworkType.fromString(preferencesHelper
					.readString(context.getString(R.string.pref_pairing_kind), NetworkType.globalDefaultAsString()));
		}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (id == R.id.action_program_and_manage) {
			...
			// Allow the user to use the Control Hub operating system's UI, instead of
			// relaunching the app
	...
	/**
	 * Updates the orientation of monitorContainer (which contains
	 * cameraMonitorView) based on the given configuration. Makes the children split
	 * the space.
	 */
	...
	private void checkPreferredChannel() {
		...
		if (networkType == NetworkType.WIFIDIRECT) {
			int prefChannel = preferencesHelper.readInt(getString(com.qualcomm.ftccommon.R.string.pref_wifip2p_channel),
					-1);
			if (prefChannel == -1) {
				prefChannel = 0;
