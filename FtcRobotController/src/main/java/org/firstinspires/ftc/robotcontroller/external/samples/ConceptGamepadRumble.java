...
public class ConceptGamepadRumble extends LinearOpMode {
	...
	public void runOpMode() {
		// Example 1. a) start by creating a three-pulse rumble sequence: right, LEFT,
		// LEFT
		customRumbleEffect = new Gamepad.RumbleEffect.Builder().addStep(0.0, 1.0, 500) // Rumble right motor 100% for
																						// 500 mSec
		...
		while (opModeIsActive()) {
			...
			// ----------------------------------------------------------------------------------------
			// Example 1. b) Watch the runtime timer, and run the custom rumble when we hit
			// half-time.
			...
			// ----------------------------------------------------------------------------------------
			// Example 2. If Left Bumper is being pressed, power the rumble motors based on
			// the two
			// trigger depressions.
			// This is useful to see how the rumble feels at various power levels.
			// ----------------------------------------------------------------------------------------
			if (currentLB) {
				// Left Bumper is being pressed, so send left and right "trigger" values to left
				// and right
				// rumble motors.
				...
				telemetry.addData("> : Rumble", "Left: %.0f%%   Right: %.0f%%", gamepad1.left_trigger * 100,
						gamepad1.right_trigger * 100);
			} else {
				// Make sure rumble is turned off when Left Bumper is released (only one time
				// each press)
				if (lastLB) {
					gamepad1.stopRumble();
				}
			...
			// ----------------------------------------------------------------------------------------
			// Example 3. Blip 3 times at the moment that A (Cross) is pressed. (look for
			// pressed
			// transition)
			...
			// Send the telemetry data to the Driver Station, and then pause to pace the
			// program.
			telemetry.update();
			sleep(10);
		}
