...
public class SensorOctoQuad extends LinearOpMode {
  ...
  private final int ODO_PERP =
      2; // Facing perpendicular direction at the center of the robot (Lateral motion)
  ...
  public void runOpMode() {
    ...
    // e.g. if you push the robot forward and the left encoder counts down, then reverse it so it
    // counts up.
    ...
    // Any changes that are made should be saved in FLASH just in case there is a sensor power
    // glitch.
  ...
  private void readOdometryPods() {
    ...
    // being sequential, we will read all of the encoder positions, and then pick out the ones we
    // need.
