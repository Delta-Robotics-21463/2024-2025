...
public class SensorOctoQuad extends LinearOpMode {
	...
	// Declare the OctoQuad object and members to store encoder positions and
	// velocities
	...
	public void runOpMode() {
		...
		// Read the Firmware Revision number from the OctoQuad and display it as
		// telemetry.
		...
		// e.g. if you push the robot forward and the left encoder counts down, then
		// reverse it so it
		// counts up.
		...
		// Any changes that are made should be saved in FLASH just in case there is a
		// sensor power
		// glitch.
	...
	private void readOdometryPods() {
		// For best performance, we should only perform ONE transaction with the
		// OctoQuad each cycle.
		// Since this example only needs to read positions from a few channels, we could
		// use either
		// readPositionRange(idxFirst, idxLast) to get a select number of sequential
		// channels
		// or
		// readAllPositions() to get all 8 encoder readings
		//
		// Since both calls take almost the same amount of time, and the actual channels
		// may not end up
		// being sequential, we will read all of the encoder positions, and then pick
		// out the ones we
		// need.
