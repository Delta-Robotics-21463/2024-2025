...
public class ConceptTelemetry extends LinearOpMode {
	...
	static final String[] poem = new String[]{"Mary had a little lamb,", "His fleece was white as snow,",
			"And everywhere that Mary went,", "The lamb was sure to go.", "", "He followed her to school one day,",
	...
	public void runOpMode() {
		...
		/*
		 * we keep track of how long it's been since the OpMode was started, just to
		 * have some interesting data to show
		 */
		 ...
		 * data is _actually_ going to make it to the driver station (recall that
		 * telemetry transmission is throttled to reduce bandwidth use. Note that
		 * getBatteryVoltage() below returns 'Infinity' if there's no voltage sensor
		 * attached.
		 *
		...
		while (opModeIsActive()) {
			...
			telemetry.addLine("left joystick | ").addData("x", gamepad1.left_stick_x).addData("y",
					gamepad1.left_stick_y);
			telemetry.addLine("right joystick | ").addData("x", gamepad1.right_stick_x).addData("y",
					gamepad1.right_stick_y);

			/*
			 * Transmit the telemetry to the driver station, subject to throttling. See the
			 * documentation for Telemetry.getMsTransmissionInterval() for more information.
			 */
			telemetry.update();
