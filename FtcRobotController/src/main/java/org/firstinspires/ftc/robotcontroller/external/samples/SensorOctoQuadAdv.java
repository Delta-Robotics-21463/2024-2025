...
/***
 * OctoSwerveDrive class manages 4 Swerve Modules - Performs general OctoQuad
 * initialization - Creates 4 module classes, one for each swerve module -
 * Updates swerve drive status by reading all data from OctoQuad and Updating
 * each module - Displays all swerve drive data as telemetry
 */
class OctoSwerveDrive {
	...
	public OctoSwerveDrive(OctoQuad octoquad) {
		...
		// Clear out all prior settings and encoder data before setting up desired
		// configuration
		octoquad.resetEverything();

		// Assume first 4 channels are relative encoders and the next 4 are absolute
		// encoders
		octoquad.setChannelBankConfig(OctoQuad.ChannelBankConfig.BANK1_QUADRATURE_BANK2_PULSE_WIDTH);

		// Create the four OctoSwerveModules, and add them to a 'list' for easier
		// reference.

		// Note: The wheel/channel order is set here. Ensure your encoder cables match.
		//
		// Note: The angleOffset must be set for each module so a forward facing wheel
		// shows a steer
		// angle of 0 degrees.
		...
		// 2) Physically rotate all the swerve drives so the wheels are facing forward
		// in the
		// desired 0 degree position
		// 3) Run the OpMode, view the telemetry and record the "Degrees" value for each
		// module.
		// 4) Update the code by entering the recorded Degrees value for each module as
		// the
		// angleOffset (last) parameter in the lines below.
		//
		// Rebuild and deploy the new code. Verify that the telemetry now indicates 0
		// degrees when the
		// wheels are facing forward.
		// Also verify that the correct module values change appropriately when you
		// manually spin
		// (drive) and rotate (steer) a wheel.
	...
	public void updateModules() {
		// Read full OctoQuad data block and then pass DataBlock to each swerve module
		// to update
		// themselves.
...
class OctoSwerveModule {
	...
	// The correct drive and turn directions must be set for the Swerve Module based
	// on the specific
	// hardware geometry.
	// Forward motion must generate an increasing drive count.
	// Counter Clockwise steer rotation must generate an increasing Steer Angle
	// (degrees)
	private static final boolean INVERT_DRIVE_ENCODER = false; // Set true if forward motion decreases drive "Count"
	private static final boolean INVERT_STEER_ENCODER = false; // Set true if counter clockwise steer action decreases
																// steer "Degree"

	/***
	 * @param octoquad
	 *            provide access to configure OctoQuad
	 * @param name
	 *            name used for telemetry display
	 * @param quadChannel
	 *            Quadrature encoder channel. Pulse Width channel is this + 4
	 * @param angleOffset
	 *            Angle to subtract from absolute encoder to calibrate zero
	 *            position. (see comments above)
	 */
	public OctoSwerveModule(OctoQuad octoquad, String name, int quadChannel, double angleOffset) {
		...
		// Set the drive encoder direction. Note the absolute encoder does not have
		// built-in direction
		// inversion.
	 ...
	 * 
	 * @param encoderDataBlock
	 *            most recent full data block read from OctoQuad.
	 */
	public void updateModule(OctoQuad.EncoderDataBlock encoderDataBlock) {
		...
		driveCountsPerSec = encoderDataBlock.velocities[channel] * VELOCITY_SAMPLES_PER_S; // convert counts/interval to
																							// counts/sec

		// convert uS to degrees. Add in any possible direction flip.
		steerDegrees = AngleUnit.normalizeDegrees(
				(encoderDataBlock.positions[channel + 4] * DEGREES_PER_US * steerDirMult) - angleOffset);
		// convert uS/interval to deg per sec. Add in any possible direction flip.
		steerDegreesPerSec = encoderDataBlock.velocities[channel + 4] * DEGREES_PER_US * steerDirMult
				* VELOCITY_SAMPLES_PER_S;
	}
	 ...
	 */
	public void show(Telemetry telemetry) {
		telemetry.addData(name, "%8.0f %7.0f %7.0f %6.0f", driveCounts, driveCountsPerSec, steerDegrees,
				steerDegreesPerSec);
	}
}
